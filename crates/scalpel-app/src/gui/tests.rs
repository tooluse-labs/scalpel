...
fn page_content_stream_node_detection_follows_contents_arrays() {
    let doc = scalpel_core::DocumentId(7);
...
fn real_tree_search_hit_row_preserves_search_node() {
    ...
    let mut tree = RealObjectTree::from_child_page(&scalpel_core::ChildPage {
        total: Some(0),
        items: Vec::new(),
    });
...
fn diagnostics_model_includes_text_page_errors_and_filters_codes() {
    ...
    app.text_search_result = Some(TextSearchResult {
        ...
        searched_pages: 1,
        cache_hits: 0,
        page_errors: vec![scalpel_core::TextSearchPageError {
            page_index: 2,
...
fn real_tree_row_anatomy_uses_kind_count_ref_preview_and_diagnostics() {
    let summary = ObjectSummary {
        ...
        child_count: Some(3),
        byte_size_hint: None,
        diagnostics: vec![scalpel_core::DiagnosticSummary {
            severity: DiagnosticSeverity::Warning,
            code: scalpel_core::DiagnosticCode::RepairWarning,
...
fn real_tree_detail_refresh_keeps_structural_label_stable() {
    ...
    let detail = ObjectDetail {
        ...
        dictionary_entries: Some(scalpel_core::ChildPage {
            total: Some(4),
            items: Vec::new(),
        }),
        array_entries: None,
...
fn real_tree_page_rows_use_dictionary_badge_before_detail_load() {
    let doc = scalpel_core::DocumentId(9);
    let summary = ObjectSummary {
...
fn real_tree_xref_object_rows_use_object_badge_before_detail_load() {
    let doc = scalpel_core::DocumentId(10);
    let summary = ObjectSummary {
        id: NodeId::XrefObject {
            doc,
            object: ObjectId { num: 4, gen: 0 },
...
fn page_index_for_node_follows_page_children() {
    ...
    assert_eq!(
        page_index_for_node(&NodeId::ResourceGroup {
            doc: scalpel_core::DocumentId(1),
            page_index: 2,
            group: scalpel_core::ResourceGroup::XObjects,
        }),
        Some(2)
    );
    assert_eq!(
...
fn page_row_node_detection_ignores_page_child_nodes() {
    let doc = scalpel_core::DocumentId(1);
    let page = NodeId::ArrayEntry {
        doc: doc.clone(),
        parent: Box::new(NodeId::PageRoot { doc: doc.clone() }),
        index: 2,
...
fn gui_open_pdf_without_real_mupdf_keeps_current_document() {
    ...
    assert!(app
        .open_pdf_error
        .as_deref()
        .is_some_and(|err| err.contains("requires building scalpel-app")));
...
fn theme_defines_named_font_stacks_and_severity_colors() {
    ...
    assert!(fonts
        .families
        .contains_key(&FontFamily::Name("scalpel-sans".into())));
    assert!(fonts
        .families
        .contains_key(&FontFamily::Name("scalpel-mono".into())));

    let style = scalpel_style();
...
fn mutool_path() -> PathBuf {
    if let Some(path) = std::env::var_os("SCALPEL_MUTOOL_PATH") {
        return PathBuf::from(path);
    }
    let source_dir = std::env::var_os("SCALPEL_MUPDF_SOURCE_DIR")
        .expect("real encrypted GUI test requires SCALPEL_MUPDF_SOURCE_DIR or SCALPEL_MUTOOL_PATH");
    ...
    assert!(
            path.is_file(),
            "real encrypted GUI test requires mutool at {}; build it with `make build=release build/release/mutool` or set SCALPEL_MUTOOL_PATH",
            path.display()
        );
    path
}
...
fn synthetic_large_xref_pdf(object_count: usize) -> Vec<u8> {
    ...
    let stream = "BT /F1 12 Tf 24 120 Td (scalpel large smoke) Tj ET";
    push_object(
        &mut pdf,
        &mut offsets,
        4,
...
fn stream_export_into_missing_directory_fails_cleanly() {
    ...
    let out_path = std::env::temp_dir()
        .join(format!("scalpel-no-such-dir-{}", std::process::id()))
        .join("export.bin");
