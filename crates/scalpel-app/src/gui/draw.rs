...
fn build_commit_label() -> &'static str {
    option_env!("SCALPEL_BUILD_COMMIT").unwrap_or("unknown")
}
...
fn read_mupdf_version_from_env() -> Option<String> {
    for path in [
        std::env::var_os("SCALPEL_MUPDF_SOURCE_DIR")
            .filter(|path| !path.is_empty())
            .map(PathBuf::from),
        std::env::var_os("SCALPEL_MUPDF_INCLUDE_DIR")
            .filter(|path| !path.is_empty())
            .map(PathBuf::from),
    ]
    .into_iter()
