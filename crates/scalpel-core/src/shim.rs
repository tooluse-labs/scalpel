...
impl FakeDoc {
    ...
    fn stream_save(
        ...
        let tmp_path = format!("{path}.scalpel-tmp");
...
impl PdbgContext {
    ...
    fn open_raw_document_handle(
        ...
        let _open_guard = self
            .open_lock
            .lock()
            .expect("scalpel context mutex poisoned");
...
mod tests {
    ...
    fn mutool_path() -> std::path::PathBuf {
        ...
        let source_dir = std::env::var_os("SCALPEL_MUPDF_SOURCE_DIR")
            .expect("real encrypted test requires SCALPEL_MUPDF_SOURCE_DIR or SCALPEL_MUTOOL_PATH");
        ...
        assert!(
            path.is_file(),
            "real encrypted test requires mutool at {}; build it with `make build=release build/release/mutool` or set SCALPEL_MUTOOL_PATH",
            path.display()
        );
        path
    }
