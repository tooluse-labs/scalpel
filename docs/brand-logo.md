...
- [`crates/scalpel-app/assets/icons/scalpel-mark.png`](../crates/scalpel-app/assets/icons/scalpel-mark.png)：带 macOS 视觉安全边距的透明背景 app icon，用于 native 窗口图标和后续打包。
...
Rust crate、binary、release 产物统一使用 `scalpel-*` / `scalpel` / `Scalpel`。
底层 C ABI 的 `pdbg_*` 符号暂时保留，用于兼容已有 shim 边界。
...
- App icon 使用带透明安全边距的 `crates/scalpel-app/assets/icons/scalpel-mark.png`，不包含字标，避免在 macOS Dock 中显得过大。
